      ...
      <div className='basket__container'>
        ...
        {items.length > 0
          ...
          <div className='basket__notice'>
            <img className='basket__notice-img' src="./images/basket-notice-null.svg" />
            <h3 className='basket__notice-title'>Корзина пустая</h3>
            <p className='basket__notice-text'>Добавьте хотя бы одну пару кроссовок, чтобы сделать заказ.</p>
            <button className='basket__notice-btn'>
              <span className='basket__btn-arrow basket__btn-arrow_notice'></span>
              <p className='basket__btn-text' onClick={onClickCloseBtn}>Вернуться назад</p>
            </button>
          </div>
        }
        {items.length > 0 &&
